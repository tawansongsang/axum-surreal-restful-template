// TODO: function for format thai timezone
use derive_more::From;
use serde::Serialize;
use serde_with::{serde_as, DisplayFromStr};
use time::{error::Format, macros::offset, Duration, OffsetDateTime};
...
pub fn format_time_thai(time: OffsetDateTime) -> Result<String> {
    let ft = time.to_offset(offset!(+7)).format(&Rfc3339)?;
    Ok(ft)
}
